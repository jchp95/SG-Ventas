...
class OffCanvasActRecientes extends React.Component {
    static open() {
    ...
    render() {
        const { show, onClose, actividades = [], verMas, actividad } = this.props;
        if (!show) return null;
        // Si hay actividad seleccionada, mostrar solo el detalle
        if (actividad) {
            return (
                <window.OffCanvas show={show} onClose={onClose} width="350px">
        ...
        return (
            <window.OffCanvas show={show} onClose={onClose} width="350px">
                ...
                {verMas && <div className="text-center mt-3"><button className="register-button" onClick={verMas}>Ver más</button></div>}
            </window.OffCanvas>
        );
    }
}
