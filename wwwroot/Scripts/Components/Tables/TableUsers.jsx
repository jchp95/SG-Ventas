function TableUsers(props) {
        ...
        return [
            ...
            {Header: 'Email', accessor: 'email'},
            {
                ...
                Cell: function ({value, row}) {
                    const rol = value && value.length > 0 ? value[0] : 'Usuario';
                    const nivel = row.original.nivel;
                    return React.createElement('span', {
                        className: `badge ${rol === 'Administrador' ? 'bg-primary' : 'bg-info'}`,
                        title: `Nivel ${nivel}`
                    }, rol);
                }
            },
    ...
    if (data.length === 0) {
        return React.createElement('div', {className: 'text-center p-5'},
            React.createElement('p', {className: 'text-muted'}, 'No hay usuarios para mostrar.')
        );
    }
